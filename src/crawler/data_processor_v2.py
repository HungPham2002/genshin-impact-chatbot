...
class EnhancedGenshinProcessor:
    ...
    def extract_clean_description(self, name: str, intro: str) -> str:
        """Extract only the meaningful character description"""
        if not intro:
            return ""

        # First, clean the text
        text = self.clean_text(intro)

        # Remove noise patterns
        for pattern in self.NOISE_PATTERNS:
            text = re.sub(pattern, '', text, flags=re.IGNORECASE)

        # Find the main description - usually starts with character name
        patterns_to_find = [
            rf'{re.escape(name)}\s+is\s+a\s+playable[^. ]+\.',
            rf'{re.escape(name)}\s+is\s+the[^.]+\.',
            rf'{re.escape(name)}\s+is\s+an?\s+[^.]+\.',
            rf'(?:A|An|The)\s+[^.]+,\s+(?:he|she)\s+is\s+[^.]+\.'
        ]

        # Also try with first name only
        first_name = name.split()[0]
        patterns_to_find.extend([
            rf'{re.escape(first_name)}\s+is\s+a\s+playable[^.]+\.',
        ])
        ...
        # Also try to find additional description sentences
        sentences = re.split(r'(?<=[.!?])\s+', text)

        meaningful_keywords = [
            'is a', 'serves as', 'works as', 'known for', 'famous for',
            'member of', 'captain of', 'leader of', 'head of', 'part of',
            'specializes in', 'skilled in', 'master of', 'expert in',
            'lives in', 'resides in', 'hails from', 'comes from',
            'personality', 'character', 'role', 'occupation', 'revealed to be',
            'former', 'current', 'consultant', 'archon', 'god', 'immortal', 'knight', 'captain',
        ]

        for sentence in sentences:
            sentence = sentence.strip()
            if len(sentence) > 30 and len(sentence) < 500:
                if any(kw in sentence.lower() for kw in meaningful_keywords):
                    if sentence not in description_parts:
                        description_parts.append(sentence)

        if description_parts:
            description = ' '.join(description_parts[: 3])  # Max 3 sentences
        else:
            # Fallback:  take clean text, remove known noise, get first meaningful part
    ...
    def process_character(self, raw_data: Dict) -> Dict:
        ...
        info['description'] = self.extract_clean_description(name, intro)
...
def main():
    ...
    # Save processed data
    processor. save_data(processed_data, "characters_processed_v2.json")

    # Create smart chunks
    chunks = processor.create_smart_chunks(processed_data)
    processor.save_data(chunks, "characters_chunks_v2.json")
